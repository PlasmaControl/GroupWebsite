...
class Publication(BaseModel):
    ...
    def author_list(self) -> str:
        ...
        if len(self.authors) > 1:
            self.authors[-1] = "and " + self.authors[-1]
    ...
    def pdf_url(self) -> str:
        ...
        if self.pdf_file_name is None:
            return None
        return f"./assets/data/publications/pdfs/{self.pdf_file_name}"
    ...
    def validate_pdf_exists(cls, pdf_file_name: str) -> str:
        ...
        if not pdf_file_path.exists() and pdf_file_path.is_file():
            message = f"The file {pdf_file_name} does not exist!"
...
class Member(BaseModel):
    ...
    def photo_url(self) -> str | None:
        ...
        if self.photo_file_name is not None:
            return f"assets/data/members/photos/{self.photo_file_name}"
        return "assets/data/members/photos/person.png"

    @functools.cached_property
    def cv_html_url(self) -> str | None:
        """Return the URL of the CV file."""
        if self.cv_file_name is not None:
            return f'<a href="../assets/data/members/cvs/{self.cv_file_name}">CV</a>'
    ...
    @functools.cached_property
    def google_scholar_html_url(self) -> str | None:
    ...
    @functools.cached_property
    def orcid_html_url(self) -> str | None:
    ...
    @classmethod
    def validate_cv_exists(cls, cv_file_name: str | None) -> str | None:
        ...
        if cv_file_name is not None:
            ...
            if not cv_file_path.exists():
                message = f"The file {cv_file_name} does not exist!"
                raise FileNotFoundError(message)
    ...
    @classmethod
    def validate_photo_exists(cls, photo_file_name: str | None) -> str | None:
        ...
        if photo_file_name is not None:
            ...
            if not photo_file_path.exists():
                message = f"The file {photo_file_name} does not exist!"
                raise FileNotFoundError(message)
...
class GroupMembers(BaseModel):
    ...
    def sort_graduate_students(cls, members) -> list[Member]:
        ...
        return sorted(alphabetical, key=lambda member: member.title, reverse=True)
...
def define_env(env):
    ...
    def year_of_research_objects():
        ...
        if not publications_file_path.exists():
            message = "The file publications.yaml does not exist!"
            raise FileNotFoundError(message)
    ...
    def members():
        ...
        if not members_file_path.exists():
            message = "The file members.yaml does not exist!"
            raise FileNotFoundError(message)
